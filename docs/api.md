...
**后台阶段:** | 阶段 | 模块 | 说明 | 条件 | |------|------|------|------| | pre_recon | `WapePreRecon` | 预侦察，分析源码提取安全架构 | 始终执行 | | recon | `WapeRecon` | 侦察，关联外部扫描与代码 | 始终执行 | | recon_graph | `ReconGraphGen` | 生成 mermaid 架构图 | 始终执行 | | auth_vuln | `WapeAuthVuln` | 认证漏洞分析 | `scan_scope` 含 `auth` | | auth_vuln_exploit | `WapeAuthVulnExploit` | 认证漏洞利用 | `scan_scope` 含 `auth` |
